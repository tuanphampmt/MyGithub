...
window.Modernizr = function (window, document, undefined) {
    ...
    var featureName, hasOwnProp, version = "2.8.3", Modernizr = {}, enableClasses = !0,
        docElement = document.documentElement, mod = "modernizr", modElem = document.createElement(mod),
        mStyle = modElem.style, inputElem = document.createElement("input"), smile = ":)", toString = {}.toString,
    ...
    return Modernizr.input || webforms(), Modernizr.addTest = function (feature, test) {
        ...
        var supportsHtml5Styles, supportsUnknownElements, version = "3.7.0", options = window.html5 || {},
 ...
 */
    function (global, factory) {
        ...
        function ajaxHandleResponses(s, jqXHR, responses) {
            for (var firstDataType, ct, finalDataType, type, contents = s.contents, dataTypes = s.dataTypes; "*" === dataTypes[0];) dataTypes.shift(), void 0 === ct && (ct = s.mimeType || jqXHR.getResponseHeader("Content-Type"));
        ...
        var deletedIds = [], slice = deletedIds.slice, concat = deletedIds.concat, push = deletedIds.push,
            indexOf = deletedIds.indexOf, class2type = {}, toString = class2type.toString,
            hasOwn = class2type.hasOwnProperty, support = {}, version = "1.11.3",
 ...
 */
            function (window) {
                ...
                var i, support, Expr, getText, isXML, tokenize, compile, select, outermostContext, sortInput,
                    ...
                    contains, expando = "sizzle" + 1 * new Date, preferredDoc = window.document, dirruns = 0, done = 0,
        ...
        jQuery.fn.ready = function (fn) {
            ...
            if (!readyList) if (readyList = jQuery.Deferred(), "complete" === document.readyState) setTimeout(jQuery.ready); else if (document.addEventListener) document.addEventListener("DOMContentLoaded", completed, !1), window.addEventListener("load", completed, !1); else {
        ...
        var nodeNames = "abbr|article|aside|audio|bdi|canvas|data|datalist|details|figcaption|figure|footer|header|hgroup|mark|meter|nav|output|progress|section|summary|time|video",
            ...
            rnoshimcache = new RegExp("<(?:" + nodeNames + ")[\\s/>]", "i"), rleadingWhitespace = /^\s+/,
        ...
        wrapMap.optgroup = wrapMap.option, wrapMap.tbody = wrapMap.tfoot = wrapMap.colgroup = wrapMap.caption = wrapMap.thead, wrapMap.th = wrapMap.td, jQuery.extend({
            clone: function (elem, dataAndEvents, deepDataAndEvents) {
                ...
                if (dataAndEvents) if (deepDataAndEvents) for (srcElements = srcElements || getAll(elem), destElements = destElements || getAll(clone), i = 0; null != (node = srcElements[i]); i++) cloneCopyEvent(node, destElements[i]); else cloneCopyEvent(elem, clone);
                ...
                for (var j, elem, contains, tmp, tag, tbody, wrap, l = elems.length, safe = createSafeFragment(context), nodes = [], i = 0; i < l; i++) if (elem = elems[i], elem || 0 === elem) if ("object" === jQuery.type(elem)) jQuery.merge(nodes, elem.nodeType ? [elem] : elem); else if (rhtml.test(elem)) {
                ...
                for (tmp && safe.removeChild(tmp), support.appendChecked || jQuery.grep(getAll(nodes, "input"), fixDefaultChecked), i = 0; elem = nodes[i++];) if ((!selection || jQuery.inArray(elem, selection) === -1) && (contains = jQuery.contains(elem.ownerDocument, elem), tmp = getAll(safe.appendChild(elem), "script"), contains && setGlobalEval(tmp), scripts)) for (j = 0; elem = tmp[j++];) rscriptType.test(elem.type || "") && scripts.push(elem);
        ...
        var getStyles, curCSS, rmargin = /^margin/, rnumnonpx = new RegExp("^(" + pnum + ")(?!px)[a-z%]+$", "i"),
        ...
        jQuery.fn.extend({
            ...
            attr: function (elem, name, value) {
                ...
                if (attrNames && 1 === elem.nodeType) for (; name = attrNames[i++];) propName = jQuery.propFix[name] || name, jQuery.expr.match.bool.test(name) ? getSetInput && getSetAttribute || !ruseDefault.test(name) ? elem[propName] = !1 : elem[jQuery.camelCase("default-" + name)] = elem[propName] = !1 : jQuery.attr(elem, name, ""), elem.removeAttribute(getSetAttribute ? name : propName)
            }, attrHooks: {
                type: {
                    set: function (elem, value) {
        ...
        var ajaxLocParts, ajaxLocation, rhash = /#.*$/, rts = /([?&])_=[^&]*/,
            rheaders = /^(.*?):[ \t]*([^\r\n]*)\r?$/gm,
                ...
                return {
                    send: function (headers, complete) {
                        ...
                        xhr.send(options.hasContent && options.data || null), callback = function (_, isAbort) {
                            ...
                            if (callback && (isAbort || 4 === xhr.readyState)) if (delete xhrCallbacks[id], callback = void 0, xhr.onreadystatechange = jQuery.noop, isAbort) 4 !== xhr.readyState && xhr.abort(); else {
    ...
    function ($) {
        ...
        var JustifiedGallery = function ($gallery, settings) {
            this.settings = settings, this.checkSettings(), this.imgAnalyzerTimeout = null, this.entries = null, this.buildingRow = {
                ...
                every: 2,
                flushed: 0
            }, this.border = settings.border >= 0 ? settings.border : settings.margins, this.maxRowHeight = this.retrieveMaxRowHeight(), this.suffixRanges = this.retrieveSuffixRanges(), this.offY = this.border, this.rows = 0, this.spinner = {
                phase: 0,
                timeSlot: 150,
        ...
        JustifiedGallery.prototype.getSuffix = function (width, height) {
            var longestSide, i;
            for (longestSide = width > height ? width : height, i = 0; i < this.suffixRanges.length; i++) if (longestSide <= this.suffixRanges[i]) return this.settings.sizeRangeSuffixes[this.suffixRanges[i]];
            ...
            for (var si in this.settings.sizeRangeSuffixes) if (this.settings.sizeRangeSuffixes.hasOwnProperty(si)) {
            ...
            var newImageSrc;
            if (this.settings.thumbnailPath) newImageSrc = this.settings.thumbnailPath(imageSrc, imgWidth, imgHeight, image); else {
                var matchRes = imageSrc.match(this.settings.extension), ext = null !== matchRes ? matchRes[0] : "";
            ...
            this.settings.cssAnimation ? ($entry.addClass("entry-visible"), callback && callback()) : ($entry.stop().fadeTo(this.settings.imagesAnimationDuration, 1, callback), $entry.find("> img, > a > img").stop().fadeTo(this.settings.imagesAnimationDuration, 1, callback))
            ...
            if (null !== $image) {
                ...
                var imageSrc = $image.attr("src"), newImageSrc = this.newSrc(imageSrc, imgWidth, imgHeight, $image[0]);
            ...
            var i, $entry, imgAspectRatio, newImgW, newImgH, justify = !0, minHeight = 0,
                availableWidth = this.galleryWidth - 2 * this.border - (this.buildingRow.entriesBuff.length - 1) * this.settings.margins,
                rowHeight = availableWidth / this.buildingRow.aspectRatio, defaultRowHeight = this.settings.rowHeight,
            ...
            if (isLastRow && "hide" === this.settings.lastRow && !justifiable) {
                for (i = 0; i < this.buildingRow.entriesBuff.length; i++) $entry = this.buildingRow.entriesBuff[i], this.settings.cssAnimation ? $entry.removeClass("entry-visible") : ($entry.stop().fadeTo(0, .1), $entry.find("> img, > a > img").fadeTo(0, 0));
                return -1
            }
            ...
            var $entry, buildingRowRes, i, settings = this.settings, offX = this.border;
            ...
            if (this.maxRowHeight && (this.maxRowHeight.isPercentage && this.maxRowHeight.value * settings.rowHeight < this.buildingRow.height ? this.buildingRow.height = this.maxRowHeight.value * settings.rowHeight : this.maxRowHeight.value >= settings.rowHeight && this.maxRowHeight.value < this.buildingRow.height && (this.buildingRow.height = this.maxRowHeight.value)), "center" === settings.lastRow || "right" === settings.lastRow) {
        ...
        JustifiedGallery.prototype.checkWidth = function () {
            ...
            var spinnerContext = this.spinner, $spinnerPoints = spinnerContext.$el.find("span");
            ...
            var newEntries;
            return norewind && null != this.lastFetchedEntry ? newEntries = $(this.lastFetchedEntry).nextAll(this.settings.selector).toArray() : (this.entries = [], newEntries = this.$gallery.children(this.settings.selector).toArray()), newEntries.length > 0 && ($.isFunction(this.settings.sort) ? newEntries = this.sortArray(newEntries) : this.settings.randomize && (newEntries = this.shuffleArray(newEntries)), this.lastFetchedEntry = newEntries[newEntries.length - 1], this.settings.filter ? newEntries = this.filterArray(newEntries) : this.resetFilters(newEntries)), this.entries = this.entries.concat(newEntries), !0
            ...
            if ("string" === $.type(settings.filter)) return a.filter(function (el) {
                var $el = $(el);
                return $el.is(settings.filter) ? ($el.removeClass("jg-filtered"), !0) : ($el.addClass("jg-filtered").removeClass("jg-visible"), !1)
            });
            if ($.isFunction(settings.filter)) {
                for (var filteredArr = a.filter(settings.filter), i = 0; i < a.length; i++) filteredArr.indexOf(a[i]) == -1 ? $(a[i]).addClass("jg-filtered").removeClass("jg-visible") : $(a[i]).removeClass("jg-filtered");
                return filteredArr
            }
            ...
            var imagesToLoad = !1, skippedImages = !1, that = this;
            $.each(this.entries, function (index, entry) {
                var $entry = $(entry), $image = that.imgFromEntry($entry);
                if ($entry.addClass("jg-entry"), $entry.data("jg.loaded") !== !0 && "skipped" !== $entry.data("jg.loaded")) if (null !== that.settings.rel && $entry.attr("rel", that.settings.rel), null !== that.settings.target && $entry.attr("target", that.settings.target), null !== $image) {
                    var imageSrc = that.extractImgSrcFromImage($image);
                    if ($image.attr("src", imageSrc), that.settings.waitThumbnailsLoad === !1) {
                        var width = parseFloat($image.attr("width")), height = parseFloat($image.attr("height"));
            ...
            for (var newSizeRngSuffixes = {0: ""}, i = 0; i < suffixRanges.length; i++) if ("string" === $.type(suffixRanges[i])) try {
            ...
            var newMaxRowHeight = {};
            if ("string" === $.type(this.settings.maxRowHeight)) this.settings.maxRowHeight.match(/^[0-9]+%$/) ? (newMaxRowHeight.value = parseFloat(this.settings.maxRowHeight.match(/^([0-9]+)%$/)[1]) / 100, newMaxRowHeight.isPercentage = !1) : (newMaxRowHeight.value = parseFloat(this.settings.maxRowHeight), newMaxRowHeight.isPercentage = !0); else {
                if ("number" !== $.type(this.settings.maxRowHeight)) {
            ...
            if (lastRowModes.indexOf(this.settings.lastRow) === -1) throw"lastRow must be one of: " + lastRowModes.join(", ");
 ...
 */
    function (root, factory) {
        ...
        var extend = function (objects) {
            var prop, extended = {}, i = 1, merge = function (obj) {
        ...
        var imgsRequired, currentlyLoaded, defaults = {columns: 3, margin: 2, trueOrder: !0, waitForImages: !1},
            cache = {options: {}}, getCurrentColumns = function () {
                var noOfColumns, docWidth = document.body.clientWidth;
                for (var widths in cache.options.breakAt) if (docWidth < widths) {
                ...
                marginIncluded = "undefined" == typeof marginIncluded || marginIncluded;
                ...
                var margin, str, noOfColumns = getCurrentColumns(), totalLeft = 0;
                ...
                if (0 === row) return 0;
                for (var i = 0; i < row; i++) tempHeight = parseInt(getProperty(cache.elements[eles[i]], "height").replace("px", ""), 10), totalHeight += isNaN(tempHeight) ? 0 : tempHeight + cache.options.margin;
                return totalHeight
            }, reOrder = function (columns) {
                var col = 0, elements2d = [], tempIndexs = [], indexArray = [];
                ...
                for (var i = 0, elements2dLen = elements2d.length; i < elements2dLen; i++) for (var eleIndexs = elements2d[i], j = 0, eleIndexsLen = eleIndexs.length; j < eleIndexsLen; j++) indexArray[j] = "undefined" == typeof indexArray[j] ? [] : indexArray[j], indexArray[j].push(eleIndexs[j]);
                ...
                for (var eles = cache.elements, element2dArray = cache.rows, i = 0, element2dArrayLen = element2dArray.length; i < element2dArrayLen; i++) for (var rowArray = alternate ? bubbleSort(element2dArray[i]) : element2dArray[i], j = 0, rowArrayLen = rowArray.length; j < rowArrayLen; j++) {
                    var left, top;
                ...
                for (var arr = list, n = arr.length - 1, i = 0; i < n; i++) for (var j = 0; j < n; j++) if (arr[j] > arr[j + 1]) {
                    var temp = arr[j];
                ...
                for (var i = 0, arrLen = arr.length; i < arrLen; i++) func(i, arr[i])
    ...
    $.fn.tilt = function (options) {
        var requestTick = function () {
            ...
            var _this2 = this;
            void 0 !== this.timeout && clearTimeout(this.timeout), $(this).css({transition: this.settings.speed + "ms " + this.settings.easing}), this.settings.glare && this.glareElement.css({transition: "opacity " + this.settings.speed + "ms " + this.settings.easing}), this.timeout = setTimeout(function () {
                $(_this2).css({transition: ""}), _this2.settings.glare && _this2.glareElement.css({transition: ""})
            }, this.settings.speed)
        }, mouseEnter = function (event) {
            this.ticking = !1, $(this).css({"will-change": "transform"}), setTransition.call(this), $(this).trigger("tilt.mouseEnter")
        }, getMousePositions = function (event) {
            return "undefined" == typeof event && (event = {
            ...
            var width = $(this).outerWidth(), height = $(this).outerHeight(), left = $(this).offset().left,
                top = $(this).offset().top, percentageX = (this.mousePositions.x - left) / width,
            ...
            if (glarePrerender || $(this).append('<div class="js-tilt-glare"><div class="js-tilt-glare-inner"></div></div>'), this.glareElementWrapper = $(this).find(".js-tilt-glare"), this.glareElement = $(this).find(".js-tilt-glare-inner"), !glarePrerender) {
                ...
                this.glareElementWrapper.css(stretch).css({overflow: "hidden"}), this.glareElement.css({
                    position: "absolute",
                    top: "50%",
                    left: "50%",
                    "pointer-events": "none",
            ...
            this.glareElement.css({width: "" + 2 * $(this).outerWidth(), height: "" + 2 * $(this).outerWidth()})
        };
        return $.fn.tilt.destroy = function () {
            ...
            this.settings = $.extend({
                ...
                $(_this4).data("settings", _this4.settings), _this4.settings.glare && prepareGlare.call(_this4), bindEvents.call(_this4)
            }, this.init()
        })
    }, $("[data-tilt]").tilt()
}(jQuery),
    ...
    function () {
        ...
        function init() {
            if (!initDone && document.body) {
                ...
                if (root = document.compatMode.indexOf("CSS") >= 0 ? html : body, activeElement = body, initTest(), top != self) isFrame = !0; else if (isOldSafari && scrollHeight > windowHeight && (body.offsetHeight <= windowHeight || html.offsetHeight <= windowHeight)) {
        ...
        function keydown(event) {
            ...
            var shift, x = 0, y = 0, overflowing = overflowingAncestor(activeElement);
        ...
        function isInsideYoutubeVideo(event) {
            ...
            if (document.URL.indexOf("www.youtube.com/watch") != -1) do if (isControl = elem.classList && elem.classList.contains("html5-video-controls")) break; while (elem = elem.parentNode);
            return isControl
        }
        ...
        var activeElement, observer, refreshSize, clearCacheTimer, deltaBufferTimer, defaultOptions = {
            ...
            arrowKeys = {37: 1, 38: 1, 39: 1, 40: 1}, que = [], pending = !1, lastScroll = Date.now(),
            uniqueID = function () {
                var i = 0;
                return function (el) {
        ...
        var wheelEvent, requestFrame = function () {
            ...
            }(), userAgent = window.navigator.userAgent, isEdge = /Edge/.test(userAgent),
            isChrome = /chrome/i.test(userAgent) && !isEdge, isSafari = /safari/i.test(userAgent) && !isEdge,
