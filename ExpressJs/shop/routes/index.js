...
router.get('/', async (req, res, next) => {
    try {
        const shoppingCart = new ShoppingCart(req.session.cart ? req.session.cart : {items: {}});
...
router.get('/cart', function (req, res, next) {
    ...
    if (req.session.cart) {
        if (countJson(req.session.cart.items) > 0) {
            return res.render('cart', {cartData: shoppingCart.convertArray(), user: req.user});
        }
        return res.redirect('/');
    }
    res.redirect('/');
...
router.get('/contact', function (req, res, next) {
    var shoppingCart = new ShoppingCart((req.session.cart) ? req.session.cart : {items: {}});
    res.render('contact', {cartData: shoppingCart.convertArray(), user: req.user});
});
router.get('/productgird', async (req, res, next) =>{
    const shoppingCart = new ShoppingCart((req.session.cart) ? req.session.cart : {items: {}});
    const data = await Product.find({});
    res.render('productgird', {products: data, cartData: shoppingCart.convertArray(), user: req.user});
});
router.get('/productlitst', async (req, res, next) =>{
    const shoppingCart = new ShoppingCart((req.session.cart) ? req.session.cart : {items: {}});
    const data = await Product.find({});
    res.render('productlitst', {products: data, cartData: shoppingCart.convertArray(), user: req.user});
});
